...
export const analyzeProvidedBazi = async (year: string, month: string, day: string, hour: string) => {
  ...
  if (!apiKey) {
    throw new Error("API Key 缺失。請檢查 Vercel 項目設置中是否已添加 API_KEY 並重新部署。");
  }
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          required: ["elementBalance", "summary", "pianCaiStrength", "pianCaiAnalysis"]
        }
      }
    });
    
    const text = response.text;
    if (!text) throw new Error("AI 未能產生分析內容");
    return JSON.parse(text);
  } catch (error: any) {
    throw new Error(`分析失敗: ${error.message || '連線至 AI 服務時發生錯誤'}`);
  }
};
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          required: ["numbers", "bettingTime", "explanation", "auspiciousDate"]
        }
      }
    });
    
    const text = response.text;
    if (!text) throw new Error("AI 未能產生號碼");
    const result = JSON.parse(text);
    if (!result.numbers || result.numbers.length === 0) throw new Error("AI 回傳的號碼無效");
    return result;
  } catch (error: any) {
    throw new Error(`號碼計算失敗: ${error.message || '請重試'}`);
  }
};